import { AED_TOTAL_STEPS } from './autoencoders-diffusionStore';

const STORAGE_KEY = 'autoencoders-diffusion-progress';
...
export function getProgressPercent(): number {
  const completed = getCompletedSteps();
  return Math.round((completed.size / AED_TOTAL_STEPS) * 100);
}
