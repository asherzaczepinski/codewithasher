import { OPT_TOTAL_STEPS } from './optimization-theoryStore';

const STORAGE_KEY = 'optimization-theory-progress';
...
export function getProgressPercent(): number {
  const completed = getCompletedSteps();
  return Math.round((completed.size / OPT_TOTAL_STEPS) * 100);
}
