import { UL_TOTAL_STEPS } from './unsupervised-learningStore';

const STORAGE_KEY = 'unsupervised-learning-progress';
...
export function getProgressPercent(): number {
  const completed = getCompletedSteps();
  return Math.round((completed.size / UL_TOTAL_STEPS) * 100);
}
