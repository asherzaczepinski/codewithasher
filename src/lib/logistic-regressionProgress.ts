import { LOG_TOTAL_STEPS } from './logistic-regressionStore';

const STORAGE_KEY = 'logistic-regression-progress';
...
export function getProgressPercent(): number {
  const completed = getCompletedSteps();
  return Math.round((completed.size / LOG_TOTAL_STEPS) * 100);
}
