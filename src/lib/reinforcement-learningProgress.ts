import { RL_TOTAL_STEPS } from './reinforcement-learningStore';

const STORAGE_KEY = 'reinforcement-learning-progress';
...
export function getProgressPercent(): number {
  const completed = getCompletedSteps();
  return Math.round((completed.size / RL_TOTAL_STEPS) * 100);
}
