...
export default function Step9() {
  return (
    <div>
      ...
      <ExplanationBox title="Why Throw the Length Away at All?">
        <p>
          It seems wasteful to compute a vector&apos;s length and then deliberately cancel it. But in most
          real uses, that length is carrying <strong>noise, not meaning</strong>. A vector tends to grow
          longer for incidental reasons — the word is very common, the text is long, the signal is simply
          &ldquo;loud&rdquo; — none of which change what it is <em>about</em>. If we kept the magnitude (the
          plain dot product), those big vectors would win comparisons just for being big, drowning out a
          shorter vector that points in exactly the right direction.
        </p>
        <p>
          Cosine sidesteps that by asking the only question we usually care about: <strong>are these two
          things pointing the same way?</strong> That is what makes it the default ruler for the jobs
          embeddings are actually used for:
        </p>
        <ul style={{ margin: '0 0 8px', paddingLeft: '1.2rem', fontSize: 14, color: '#475569', lineHeight: 1.7 }}>
          <li><strong>Search &amp; retrieval</strong> — a one-line query should still match a long article on the same topic, even though the article&apos;s vector is far &ldquo;bigger.&rdquo;</li>
          <li><strong>Recommendations &amp; clustering</strong> — group items by what they&apos;re about, not by how popular or how long they happen to be.</li>
          <li><strong>Comparing rare and common words</strong> — a word that appears once and a word that appears a million times can still be judged similar in meaning.</li>
        </ul>
        <p>
          Keep the magnitude only when size genuinely <em>is</em> the signal (say, how strongly a feature
          fires). For &ldquo;do these mean the same thing?&rdquo; — direction is the answer, so cosine is
          the tool.
        </p>
      </ExplanationBox>
