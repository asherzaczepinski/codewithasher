  ...
  return (
    <div>
      ...
      <ExplanationBox title="Why Squared Error?">
        <p>
          We could just use the plain distance — how far apart the prediction and target are,
          ignoring direction — but we square it instead. Here&apos;s why:
        </p>
        <p>
      ...
      <ExplanationBox title="Loss Over the Whole Dataset">
        <p>
          We have many training days — each with weather inputs and a known rain outcome. We work out
          the loss for <em>every</em> day the same way, then take the <strong>average</strong> across
          all of them: run each day&apos;s inputs through the network, compare the guess to what
          actually happened, square the gap, and average those squared gaps together.
        </p>
        <p style={{ marginTop: '1rem' }}>
          That single average is how well the network is doing <em>overall</em>, not just on one lucky
          or unlucky day. Training aims to make that average as small as possible.
        </p>
      </ExplanationBox>
