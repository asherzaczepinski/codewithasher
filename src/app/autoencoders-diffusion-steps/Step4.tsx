  ...
  return (
    <div>
      ...
      <ExplanationBox title="In Python">
        <p>
          The snippet below implements the three ideas unique to a VAE: the encoder outputs
          <code>mu</code> and <code>logvar</code> instead of a single vector; the
          reparameterization trick samples <code>z</code> differentiably; and the loss adds a
          KL term to the MSE. This is <strong>illustrative</strong> PyTorch code — comments
          map directly to the math above.
        </p>
      </ExplanationBox>

      <CodeBlock
        filename="vae.py"
        caption="VAE encoder outputting mu and logvar, the reparameterization trick, and the combined ELBO loss."
        code={`import torch
...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim=784, latent_dim=8):
        ...
        # ENCODER — shared trunk that extracts features from the input.
        ...
        # TWO HEADS — instead of one output vector the encoder produces
        # two separate vectors of the same size as the latent space.
        self.fc_mu     = nn.Linear(64, latent_dim)  # mean of q(z|x)
        self.fc_logvar = nn.Linear(64, latent_dim)  # log-variance of q(z|x)
        # We predict log(sigma^2) rather than sigma^2 because:
        #   - it is unconstrained (any real number is valid)
        #   - exp() is always positive, keeping sigma^2 > 0
        #   - gradients are numerically better behaved near zero

        # DECODER — identical to the plain autoencoder decoder.
        self.decoder = nn.Sequential(
            ...
            nn.Sigmoid(),  # pixel values must be in [0, 1]
        )
    ...
    def reparameterize(self, mu, logvar):
        ...
        #   z = sample from N(mu, sigma^2)   <-- gradient cannot pass through this
        #
        # Reparameterized (differentiable):
        #   eps ~ N(0, 1)                     <-- randomness is isolated here
        #   z   = mu + std * eps              <-- gradient flows through mu and std

        # Convert log-variance to standard deviation:
        #   logvar = log(sigma^2)
        #   sigma  = exp(0.5 * logvar)        <-- always positive
        std = torch.exp(0.5 * logvar)

        # Sample epsilon from the standard normal.
        # .detach() is NOT needed — eps has no parameters, so no gradient
        # accumulates through it. The gradient of loss w.r.t. mu and std
        # flows through the addition and multiplication below.
        eps = torch.randn_like(std)  # same shape and device as std

        z = mu + std * eps  # gradient reaches mu AND std (via std * eps)
        return z

    def forward(self, x):
        # Encode: extract features, then split into mu and logvar.
        h      = self.encoder_trunk(x)
        mu     = self.fc_mu(h)      # shape: (B, latent_dim)
        logvar = self.fc_logvar(h)  # shape: (B, latent_dim)

        # Sample a latent code using the reparameterization trick.
        z = self.reparameterize(mu, logvar)  # shape: (B, latent_dim)

        # Decode the sampled z back to pixel space.
        x_hat = self.decoder(z)  # shape: (B, input_dim)

        # Return everything the loss function needs.
        return x_hat, mu, logvar


def vae_loss(x, x_hat, mu, logvar):
    # ---- Term 1: Reconstruction loss (same as the plain autoencoder) ----
    # F.mse_loss sums over all pixels and averages over the batch.
    reconstruction = F.mse_loss(x_hat, x, reduction="mean")

    # ---- Term 2: KL divergence KL( N(mu, sigma^2) || N(0, 1) ) ----
    # Closed-form formula for Gaussian KL (no integration required):
    #   KL = -0.5 * sum_j (1 + logvar_j - mu_j^2 - exp(logvar_j))
    # Summing over latent dimensions j, then averaging over the batch.
    kl = -0.5 * torch.mean(1 + logvar - mu.pow(2) - logvar.exp())
    # mu.pow(2)    = mu^2
    # logvar.exp() = sigma^2  (since logvar = log(sigma^2))

    # The two terms pull in opposite directions:
    #   reconstruction pushes each image to a distinct latent region.
    #   kl            pulls everything back toward N(0,1) near the origin.
    # The balance between them shapes the latent space.
    return reconstruction + kl


# Quick smoke test.
if __name__ == "__main__":
    model = Autoencoder_VAE = VAE(input_dim=784, latent_dim=8)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    x_fake = torch.rand(32, 784)  # fake batch of 32 flattened images

    x_hat, mu, logvar = model(x_fake)
    loss = vae_loss(x_fake, x_hat, mu, logvar)
