...
import { useState, useEffect, useRef, useCallback, memo, type ComponentType } from 'react';
...
export interface CourseConfig {
  /** URL base for this course, e.g. "/neural-networks" */
  basePath: string;
  /** Display name shown on the certificate, e.g. "Neural Networks" */
  courseName: string;
  /** Certificate subtitle, e.g. "Building Neural Networks from Scratch" */
  courseSubtitle: string;
  /** One-paragraph description used on the certificate */
  certificateDescription: string;
  /** Lines printed in the body of the downloadable PNG certificate */
  certificateCanvasLines: string[];
  /** Instructor name printed on the certificate */
  instructor: string;
  /** Credential ID prefix, e.g. "CWA-NN" */
  credentialPrefix: string;
  steps: CourseStep[];
  totalSteps: number;
  partNames: Record<number, string>;
  /** One lazy component per step, index 0 == step 1 */
  stepComponents: ComponentType[];
  /** Optional prefetch hook for a step number */
  preloadStep?: (stepNum: number) => void;
  progress: {
...
function CourseContent({ config }: { config: CourseConfig }) {
  const { basePath, steps: STEPS, totalSteps: TOTAL_STEPS, partNames: PART_NAMES, stepComponents } = config;
  const { getCompletedSteps, markStepComplete, markStepIncomplete } = config.progress;

  const PARTS = Object.keys(PART_NAMES)
    .map(Number)
    .sort((a, b) => a - b)
    .map(part => ({ part, name: PART_NAMES[part], steps: STEPS.filter(s => s.part === part) }));
  ...
  useEffect(() => { setCompletedArr([...getCompletedSteps()]); }, [getCompletedSteps]);
  const refreshCompleted = useCallback(() => { setCompletedArr([...getCompletedSteps()]); }, [getCompletedSteps]);
  ...
  useEffect(() => {
    if (!stepParam) {
      window.history.replaceState({}, '', `${basePath}?step=${currentStep}`);
    }
  }, []);
  ...
  useEffect(() => {
    config.preloadStep?.(currentStep + 1);
    config.preloadStep?.(currentStep - 1);
  }, [currentStep]);

  useEffect(() => {
    if (completed.has(currentStep)) setReachedBottom(true);
      ...
      if (!scrollTicking.current) {
        requestAnimationFrame(() => {
          ...
          if (footerRef.current) {
            ...
            if (rect.top <= window.innerHeight) setReachedBottom(true);
          }
  ...
  useEffect(() => {
    if ((sidebarOpen && isMobile) || showCertificate) document.body.style.overflow = 'hidden';
  ...
  const goToStep = useCallback((stepId: number) => {
    ...
    window.history.pushState({}, '', `${basePath}?step=${stepId}`);
    setSidebarOpen(false);
  }, [basePath]);

  const nextStep = useCallback(() => { if (currentStep < TOTAL_STEPS) goToStep(currentStep + 1); }, [currentStep, goToStep, TOTAL_STEPS]);
  const prevStep = useCallback(() => { if (currentStep > 1) goToStep(currentStep - 1); }, [currentStep, goToStep]);
  ...
  const downloadCertificate = () => {
    ...
    ctx.fillText(config.courseName, 700, 280);

    ctx.fillStyle = '#666';
    ctx.font = '400 20px -apple-system, BlinkMacSystemFont, sans-serif';
    ctx.fillText(config.courseSubtitle, 700, 330);
    ...
    ctx.fillStyle = '#666';
    ctx.font = '400 18px -apple-system, BlinkMacSystemFont, sans-serif';
    config.certificateCanvasLines.forEach((line, i) => {
      ctx.fillText(line, 700, 580 + i * 30);
    });
    ...
    ctx.fillStyle = '#888';
    ctx.font = '400 16px -apple-system, BlinkMacSystemFont, sans-serif';
    ctx.fillText(config.instructor, 1000, 780);
    ...
    const credId = `${config.credentialPrefix}-${Date.now().toString(36).toUpperCase()}`;
    ctx.fillStyle = '#bbb';
    ctx.font = '400 12px -apple-system, BlinkMacSystemFont, sans-serif';
    ctx.fillText(`Credential ID: ${credId}`, 700, 945);

    const link = document.createElement('a');
    link.download = `${config.courseName.replace(/\s+/g, '-')}-Certificate-${name.replace(/\s+/g, '-')}.png`;
            ...
            {currentStep < TOTAL_STEPS ? (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (nowCompleted.size === TOTAL_STEPS) setShowCertificate(true);
                }}
              >
...
export default function CourseShell({ config }: { config: CourseConfig }) {
  ...
  if (!mounted) {
    return (
      <div className="course-page">
        ...
        <span className="top-bar-title" style={{ opacity: 0.3 }}>{config.courseName}</span>
  ...
  return <CourseContent config={config} />;
}
